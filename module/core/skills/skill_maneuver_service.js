/**
 * Service for general Static Maneuver Table T-4.3 lookups (skills, not spells).
 */
export default class SkillManeuverService {
    ...
    static async loadTable() {
        ...
        try {
            const response = await fetch(`systems/rmss/module/core/skills/tables/${lang}/staticManeuverTable.json`);
            if (!response.ok) {
            ...
            console.error("Error loading skill maneuver table:", error);
            return null;
        }
    }
     ...
     * @returns {Promise<Object>} The result object with name and description
     */
